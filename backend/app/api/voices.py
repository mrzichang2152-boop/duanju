...
from app.services.linkapi import resolve_kling_auth_token
...
async def _delete_kling_custom_voice_remote(api_key: str, voice_id: str) -> bool:
    vid = str(voice_id or "").strip()
    if not vid:
        return True
    endpoint_candidates = [
        str(os.getenv("KLING_CUSTOM_VOICE_DELETE_ENDPOINT") or "").strip(),
        "https://api-beijing.klingai.com/v1/general/delete-voices",
        "https://api.magic666.cn/api/v1/general/delete-voices",
    ]
    endpoint_candidates = [item for item in endpoint_candidates if item]
    headers = {
        "Authorization": f"Bearer {api_key}",
    ...
    async with httpx.AsyncClient(timeout=30.0, trust_env=True) as client:
        for endpoint in endpoint_candidates:
            try:
                ...
                logger.warning("Delete kling custom voice request failed endpoint=%s err=%s", endpoint, exc)
                continue
            if response.status_code != 200:
                continue
            try:
...
async def _cleanup_redundant_character_voices(
    db: AsyncSession,
    project_id: str,
    character_name: str,
    kling_jwt: str,
    keep_voice_row_id: str,
    keep_voice_id: str,
) -> None:
    ...
    rows.sort(key=lambda row: (row.updated_at or row.created_at or 0, row.created_at or 0), reverse=True)
    keep_row = next((row for row in rows if str(row.id) == str(keep_voice_row_id)), rows[0])
    keep_vid = str(keep_voice_id or "").strip()

    delete_voice_ids: set[str] = set()
    for row in rows:
        if row is keep_row:
            continue
        old_vid = str(row.voice_id or "").strip()
        old_type = str(row.voice_type or "").strip().upper()
        if old_vid and old_type == "KLING_CUSTOM" and old_vid != keep_vid:
            delete_voice_ids.add(old_vid)
        await db.delete(row)

    if kling_jwt:
        for vid in delete_voice_ids:
            deleted = await _delete_kling_custom_voice_remote(kling_jwt, vid)
            if not deleted:
                logger.warning("删除历史 Kling 音色失败 project_id=%s character=%s voice_id=%s", project_id, character_name, vid)
...
async def update_character_voice(
    ...
    prev_voice_type = ""
    if voice:
        prev_voice_id = str(voice.voice_id or "").strip()
        prev_voice_type = str(voice.voice_type or "").strip().upper()
    ...
    await db.flush()
    await _cleanup_redundant_character_voices(
        db,
        project_id,
        character_name,
        kling_jwt,
    ...
    if prev_voice_type == "KLING_CUSTOM" and prev_voice_id and prev_voice_id != str(voice.voice_id or "").strip() and kling_jwt:
        deleted = await _delete_kling_custom_voice_remote(kling_jwt, prev_voice_id)
        if not deleted:
            logger.warning("删除旧 Kling 音色失败 project_id=%s character=%s voice_id=%s", project_id, character_name, prev_voice_id)
...
async def upload_character_voice_sample(
    ...
    if prev_voice_type == "KLING_CUSTOM" and prev_voice_id and prev_voice_id != voice_id:
        deleted = await _delete_kling_custom_voice_remote(kling_jwt, prev_voice_id)
        if not deleted:
            logger.warning("删除旧 Kling 音色失败 project_id=%s character=%s voice_id=%s", project_id, character_name, prev_voice_id)
