...
2.  **零污染 (No Pollution)**：
    -   严禁在项目根目录生成临时文件。
    -   所有 AI 生成的中间产物、日志、测试脚本必须存放在 `.vibecoding/` 或 `tmp/` 目录中（这些目录已在 `.gitignore` 中排除）。
3.  **模块解耦 (Decoupling)**：
...
-   **语言**：所有沟通与文档必须使用**中文 (简体)**。

## 5. 部署与环境 (Deployment)

-   **环境变量**：所有敏感信息（API Key, DB Password）必须通过环境变量注入，严禁硬编码。
